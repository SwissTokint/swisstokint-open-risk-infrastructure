...
This shared Core block owns reference semantics for independently observing an execution and reconciling the observed action/context/effect against an exact authorization binding. Application adapters may supply domain-specific observation data, but they must not fork these comparison semantics.
