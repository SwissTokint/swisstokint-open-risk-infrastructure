...
function assertProfile(value, field, code = 'POMRX_OBS_E_INVALID') {
...
function normalizeObserved(value, trustedNow) {
  ...
  return Object.freeze({
    ...
    binding_profile: assertProfile(
      value.binding_profile,
      'observed binding_profile',
      'POMRX_OBS_E_OBSERVER_INVALID',
    ),
...
export function createReferenceObservationReconciliation(options) {
  ...
  async function captureAndReconcile(input) {
    ...
    sampleTrustedClock();
    const observedSnapshot = await captureObserverOutput(observationRef);
