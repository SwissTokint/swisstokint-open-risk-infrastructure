...
The composition creates its durable store internally from `rootDir` rather than accepting a caller-supplied structural store object. Bootstrap capture rejects Proxy, accessor, symbol, hidden/unknown and custom-prototype substitution before callback/root values are accepted. Reflection and local WeakMap dispatch used by this new boundary are captured at module initialization. The composed durable claim store and process-local Gate likewise capture their load-bearing exact-object creation/reflection and WeakMap constructor/dispatch at their own module initialization. The durable store additionally binds capability/hash RegExp dispatch plus the `node:path` operations used to validate, normalize and construct paths, so post-import mutation of the mutable path export or `RegExp.prototype.test` cannot redirect the configured root or admit traversal-shaped capability identifiers. The shared `captureReferencePlainData()` boundary used by the inner Gate for trusted observer `prepared_execution` data also binds its load-bearing reflection, Proxy/array classification, output-construction/freezing, key-validation and safe-integer intrinsics at module initialization. A post-import mutation therefore cannot silently substitute the inert prepared-execution snapshot after trusted observation. Poisoning before module initialization remains outside the stated guarantee.
