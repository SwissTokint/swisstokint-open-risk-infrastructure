...
- `core/observation/` — reference independent observation and exact binding/effect reconciliation semantics.
...
Application blocks may normalize their own domain intents/context and own downstream adapters, but they must not duplicate canonicalization, hashing, verifier, Witness or Gate semantics. Observation and reconciliation are also shared Core semantics and must not be duplicated or forked by application blocks. Exact authorization is also common Core behavior and must not be forked into an application-specific implementation.
