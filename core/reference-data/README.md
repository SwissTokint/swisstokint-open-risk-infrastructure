...
Adoption by an existing Core or application module remains an explicit reviewed integration step because each caller still owns semantic validation, tighter field-specific limits and compatibility commitments. The current Node reference runtime and its trusted built-in intrinsics remain part of the installation boundary; this helper does not claim resilience to a generally compromised JavaScript realm.
