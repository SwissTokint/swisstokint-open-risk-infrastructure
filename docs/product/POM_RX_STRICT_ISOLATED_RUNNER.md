...
This reduces preload/loader/environment attack surface. It does not prove the Node binary or OS itself is trustworthy.
...
- explicit false non-claims.

The parent validates the complete result before returning it. A malformed child response, wrong scenario correlation, wrong policy identity, inconsistent conformant result, timeout or non-zero child exit fails closed.
...
- valid strict conformance in the isolated child;
...
- non-propagation of inherited `NODE_OPTIONS`;
