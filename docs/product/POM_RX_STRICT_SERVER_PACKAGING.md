...
- `sdk/typescript/pom-rx-strict-package.mjs` — package/provenance metadata, artifact host pins, and an integrity check for the complete measured closure.
...
2. call `verifyPomRxStrictPackageIntegrity()` and require `package_integrity === "verified"`;
...
`tests/pom-rx-strict-package.node.test.mjs` binds these values to the actual installed files and also runs `npm pack --dry-run` to prove that every artifact-manifest entry is present in the distributable package.
