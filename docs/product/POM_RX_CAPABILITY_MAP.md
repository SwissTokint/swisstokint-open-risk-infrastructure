...
live GitHub currently reports PR #97 not mergeable, so trusted-main
reconciliation is now an additional release requirement. Canonical exact-head CI
run `32487036517` / CI run 592 completed `success` on exact head `0efb462...`, but
...
permit reference authorization and sensitive forwarding. The exact-head
...
runtime repair on top of the then-current trusted main, prevention of inherited
...
`62edca5d...`; live GitHub currently reports the PR not mergeable. The latest
distinct Codex release evidence also covers a moved head. PR #93 therefore
