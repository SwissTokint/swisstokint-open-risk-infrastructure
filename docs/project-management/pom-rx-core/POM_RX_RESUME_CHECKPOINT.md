...
Live GitHub advanced from the PR #124 checkpoint recorded in the versioned files to assured PR #125 main `1989bb88ae2eee6ae32328f2df4cc056c0dd27d4`. This bounded non-Tier-B reconciliation persists that live state and records a new architecture constraint discovered while falsifying the proposed PR #120 repair direction.
...
Any chosen repair still requires a CI-wired `--unhandled-rejections=strict` regression proving zero reference authorization and zero sensitive forwarding for the non-configurable unsafe data-constructor case.
...
1. Complete this scoped post-PR125 control-plane reconciliation through its applicable exact-head gates; do not change runtime in this lot.
2. After this reconciliation is trusted, perform a bounded read-only architecture/security decision for PR #120's rejected-Promise transport boundary before any new runtime write.
3. Do not accept a repair that merely reorders `Promise.prototype.then`; require evidence that the selected boundary prevents an orphaned strict rejection without executing hostile constructor/species/Proxy code or weakening fail-closed behavior.
4. Once a design is selected, reconcile the PR #120 stream to then-current exact trusted main using one writer, implement the smallest repair plus strict regression, and freeze the moved head.
5. Require fresh canonical exact-head CI, the mandatory five-stage owner gate with concrete Promise/species/accessor/Proxy/strict-unhandled/thenable/Array-poisoning hypotheses, and a fresh genuinely distinct exact-head `chatgpt-codex-connector` review.
