...
test('valid resolved observer data forwards one detached frozen prepared snapshot', async () => {
  let evidence;
  const prepared = { request: 'control', nested: { amount: 1 } };
  ...
  const forwarded = setup.stats().downstreamArgument;
  assert.equal(setup.stats().downstreamCalls, 1);
  assert.notEqual(forwarded, prepared);
  assert.notEqual(forwarded.nested, prepared.nested);
  assert.equal(Object.isFrozen(forwarded), true);
  assert.equal(Object.isFrozen(forwarded.nested), true);
  prepared.nested.amount = 999;
  assert.equal(forwarded.nested.amount, 1);
});
...
test('bootstrap Proxy wrappers are rejected before reflective traps run', () => {
  ...
  assert.throws(
    ...
    /exact plain data object/u,
  );
  assert.equal(trapCalls, 0);
});

test('resolved observer field accessors are rejected without invocation', async () => {
  let getterCalls = 0;
  let evidence;
  const observed = {
    binding_profile: 'pom-rx-core-reference/0.1',
...
test('resolved observer Proxy is rejected before Core reflective traversal', async () => {
  ...
  assert.equal(reflectionTraps, 0);
  assert.equal(setup.stats().downstreamCalls, 0);
});

test('nested prepared Proxy is rejected before Core reflective traversal and never forwarded', async () => {
  let reflectionTraps = 0;
  let evidence;
