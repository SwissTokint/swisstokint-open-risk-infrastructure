...
test('unrelated canonicalizer runtime TypeError is not converted to unknown evidence', () => {
  const { recorder: evidenceRecorder, testAuthority } = recorder();
  const handle = evidenceRecorder.begin({ authorization_binding: binding(10) });
  ...
  try {
    ...
    assert.throws(
      () => evidenceRecorder.complete(handle, {
        execution_status: 'success',
        effect: { note: 'safe' },
      }),
