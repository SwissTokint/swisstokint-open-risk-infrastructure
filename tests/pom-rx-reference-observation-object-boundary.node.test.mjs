...
test('bootstrap captures trusted callbacks once and ignores later caller mutation', async () => {
  ...
  options.observeExecution = async () => {
    replacementCalls += 1;
    return { ...observed(), execution_status: 'error' };
  };
...
test('bootstrap and capture input Proxy wrappers fail before traps execute', async () => {
  ...
  assert.throws(
    () => createReferenceObservationReconciliation(proxyWithTrapCounter({
      ...
      observeExecution: async () => observed(),
    }, bootstrapCounter)),
...
test('expected_execution_status accessor cannot validate as success then downgrade to any', async () => {
  let reads = 0;
  let observerCalls = 0;
...
test('observer Proxy output is rejected before traps execute', async () => {
  const counter = { calls: 0 };
  let clockIndex = 0;
  const clock = ['2026-08-20T05:00:11.000Z', '2026-08-20T05:00:13.000Z'];
  const runtime = createReferenceObservationReconciliation({
    trustedClock: () => clock[Math.min(clockIndex++, clock.length - 1)],
    observeExecution: async () => proxyWithTrapCounter(observed(), counter),
  });
