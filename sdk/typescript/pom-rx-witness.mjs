...
const KEY_ID_PATTERN = /^ed25519-[a-f0-9]{32}$/;
...
function toEd25519PublicKey(key) {
  const keyObject = key?.type === 'public'
    ? key
...
export function createPomRxSourceEnvelope(receipt, sourcePrivateKey) {
  ...
  const publicKey = toEd25519PublicKey(sourcePrivateKey);
  ...
  return {
    ...
    source_public_key: exportPublicKey(publicKey),
...
export function verifyPomRxSourceEnvelope(envelope) {
  try {
    ...
    assert(
      envelope.schema_version === POM_RX_SOURCE_ENVELOPE_VERSION,
      'Unsupported POM-RX source envelope version',
    );
    assertHash(envelope.receipt_hash, 'receipt_hash');
    assert(
      typeof envelope.source_signature === 'string'
        && envelope.source_signature.length >= 80
        && envelope.source_signature.length <= 128
        && BASE64_PATTERN.test(envelope.source_signature),
      'Source signature must be bounded base64',
    );
    ...
    assert(
      committed.receipt.source_key_id === sourceKeyId,
      'Source public key does not match receipt source_key_id',
    );
    const signatureValid = crypto.verify(
      null,
      Buffer.from(sourceSignatureMessage(envelope.receipt_hash), 'utf8'),
      publicKey,
      Buffer.from(envelope.source_signature, 'base64'),
    );
...
export function createPomRxWitnessAck(
  ...
  const payload = {
    ...
    mode,
    witness_key_id: pomRxKeyId(witnessPublicKey),
  };
...
export function verifyPomRxWitnessAck(ack, expected = {}) {
  try {
    ...
    assert(
      validityMs >= 1_000 && validityMs <= 300_000,
      'Witness acknowledgement validity must be between 1 second and 5 minutes',
    );
    assert(
      typeof ack.witness_signature === 'string'
        && ack.witness_signature.length >= 80
        && ack.witness_signature.length <= 128
        && BASE64_PATTERN.test(ack.witness_signature),
      'Witness signature must be bounded base64',
    );
    ...
    const signatureValid = crypto.verify(
      ...
      Buffer.from(witnessSignatureMessage(payload), 'utf8'),
      publicKey,
      Buffer.from(ack.witness_signature, 'base64'),
    );
    ...
    if (expected.requireUnexpired === true) {
      const currentTime = normalizeDateTime(
        expected.currentTime ?? new Date().toISOString(),
        'currentTime',
      );
